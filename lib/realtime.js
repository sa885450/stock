...
async function fetchRealtimeData(symbols) {
    ...
    try {
        ...
        if (response.data && response.data.msgArray && response.data.msgArray.length > 0) {
            ...
            }).filter(item => item.price > 0 && item.name && !item.name.includes('undefined'));
        }
    } catch (e) {
        console.warn('⚠️ [交易所] 連線異常:', e.message);
    }

    // 2. 🛡️ 強制備援邏輯：如果證交所沒回來、或回來後完全沒效資料，則請求 Yahoo
    if (results.length === 0) {
        ...
        try {
            ...
            results = yahooResults.filter(r => r !== null && r.price > 0);
        } catch (err) {
            console.error('❌ [備援] Yahoo 抓取也失敗:', err.message);
        }
    }
