...
const { fetchRealtimeData } = require('./lib/realtime'); // ✨ 新增
...
app.get('/api/warrants/hot', async (req, res) => {
  try {
    ...
    const topWarrants = await IntradaySnapshot.find({ symbol: { $in: warrants } })
      .sort({ volume: -1 }).limit(50).lean();

    const targetSymbols = topWarrants.map(w => w.symbol);
    if (targetSymbols.length === 0) return res.json({ success: true, data: [] });
